...
export default function AdminTopBar() {
  ...
  const commandStats = useQuery(api.ai.getCommandCenterStats, {});
    ...
    <>
      <header className="sticky top-0 z-30 bg-[#FFFFFF] border-b border-[#DED3C7] text-[#241F1B] select-none shadow-xs">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 h-[64px] sm:h-[70px] flex items-center justify-between gap-4">
          ...
          {/* Right: Ask AI, Issues Alert Pill & Operator Status */}
          <div className="flex items-center gap-2 sm:gap-3">
            <button
              type="button"
              onClick={() => setIsAssistantOpen(true)}
              className="px-3 py-1.5 rounded-xl bg-[#C93B2F] hover:bg-[#A92E25] text-white text-[12.5px] font-bold flex items-center gap-1.5 transition-colors cursor-pointer shadow-xs"
            >
