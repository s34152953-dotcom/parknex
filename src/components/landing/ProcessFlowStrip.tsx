...
export default function ProcessFlowStrip() {
  return (
    <section className="relative z-20 w-full bg-[#07090D] border-t border-white/[0.05] py-4.5 px-8 sm:px-12 lg:px-16 overflow-x-auto scrollbar-none">
        ...
        {steps.map((step, idx) => (
          <div key={step.label} className="flex items-center gap-3 shrink-0">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-lg bg-[#0E131F] border border-white/[0.08] flex items-center justify-center text-white/70">
            ...
            {idx < steps.length - 1 && (
              <ArrowRight className="w-3.5 h-3.5 text-white/30 mx-1 shrink-0" />
            )}
          </div>
        ))}
      </div>
